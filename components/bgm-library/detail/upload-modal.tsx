...
export function UploadModal({
  ...
  const uploadFile = React.useCallback(
    async (uploadFile: UploadFile): Promise<boolean> => {
      ...
      try {
        ...
        const response = await bgmLibraryService.uploadBgm(
          uploadFile.file,
          {
            folder_id: folderId,
  ...
  const getStatusIcon = (file: UploadFile) => {
    switch (file.status) {
      case "queued":
        return <HugeiconsIcon icon={MusicNote01Icon} className="size-4 text-muted-foreground" />
      case "uploading":
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent style={{ maxWidth: 585 }}>
        <AlertDialogHeader>
          <AlertDialogTitle>Upload BGM</AlertDialogTitle>
          <AlertDialogDescription>
            Upload up to {MAX_FILES} audio files. Files will be processed in batches of {CONCURRENT_UPLOADS}.
          </AlertDialogDescription>
        </AlertDialogHeader>
        ...
        <div className="flex flex-col gap-4">
          ...
          {!isUploading && (
            ...
            >
              ...
              <p className="text-sm font-medium mb-1">Drag & drop audio files here</p>
