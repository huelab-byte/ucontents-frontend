...
import type { ImageStats } from "@/lib/api/services/image-library.service"
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mr-0 sm:mr-[26px]">
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">{stats?.total_image ?? 0}</div>
        </CardContent>
      </Card>
      
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">Ready Images</CardTitle>
          <HugeiconsIcon icon={CheckmarkCircle01Icon} className="size-4 text-muted-foreground" />
        </CardHeader>
        <CardContent>
          <div className="text-2xl font-bold">{stats?.ready_image ?? 0}</div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">{stats?.processing_image ?? 0}</div>
        </CardContent>
      </Card>
