...
export function ScheduledContentSection({
  ...
  // Initialize scheduled content if empty
  React.useEffect(() => {
    if (scheduledContent.length === 0) {
      const initialContent: ScheduledContent[] = (() => {
      ...
      setScheduledContent(initialContent)
    }
  }, [setScheduledContent, scheduledContent.length])
                      ...
                      <div className="size-16 rounded-lg bg-muted flex items-center justify-center shrink-0 overflow-hidden">
                        {item.type === "video" ? (
                          <div className="relative w-full h-full">
                            ...
                            <HugeiconsIcon icon={PlayIcon} className="size-6 text-primary absolute inset-0 m-auto" />
                          </div>
                        ) : item.type === "image" ? (
