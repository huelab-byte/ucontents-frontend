...
import { footageLibraryService, toast } from "@/lib/api"

export type MetadataSource = "title" | "frames" | "none"
...
export function UploadModal({
  ...
  const [metadataSource, setMetadataSource] = React.useState<MetadataSource>("title")
  ...
  const uploadFile = React.useCallback(
    async (uploadFile: UploadFile): Promise<boolean> => {
      ...
      try {
        ...
        const response = await footageLibraryService.uploadFootage(
          uploadFile.file,
          {
            folder_id: folderId,
  ...
  const getStatusIcon = (file: UploadFile) => {
    switch (file.status) {
      case "queued":
        return <HugeiconsIcon icon={FileVideoIcon} className="size-4 text-muted-foreground" />
      case "uploading":
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent style={{ maxWidth: 585 }}>
        <AlertDialogHeader>
          <AlertDialogTitle>Upload Videos</AlertDialogTitle>
          <AlertDialogDescription>
            Upload up to {MAX_FILES} video files. Files will be processed in batches of {CONCURRENT_UPLOADS}.
            {files.length > 50 && metadataSource === "frames" && (
              <span className="block mt-1 text-yellow-600">
                Tip: For large batches, consider selecting &quot;None&quot; for faster uploads. You can generate metadata later.
              </span>
            )}
          </AlertDialogDescription>
        </AlertDialogHeader>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="space-y-3">
            <Label className="text-sm font-medium">Metadata Generation</Label>
            <RadioGroup
              value={metadataSource}
              onValueChange={(value) => value && setMetadataSource(value as MetadataSource)}
              className="flex flex-wrap gap-4"
              disabled={isUploading}
            >
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="title" id="metadata-title" />
                <Label htmlFor="metadata-title" className="font-normal cursor-pointer">
                  From Title (AI generates metadata from filename)
                </Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="frames" id="metadata-frames" />
                <Label htmlFor="metadata-frames" className="font-normal cursor-pointer">
                  From Frames (AI analyzes video frames)
                </Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="none" id="metadata-none" />
                <Label htmlFor="metadata-none" className="font-normal cursor-pointer">
                  None (Skip AI metadata)
                </Label>
              </div>
            </RadioGroup>
          </div>
