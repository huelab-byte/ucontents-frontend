  ...
  const uploadFile = React.useCallback(
    async (uploadFile: UploadFile): Promise<boolean> => {
      ...
      try {
        ...
        const response = await videoOverlayService.uploadVideoOverlay(
          uploadFile.file,
          {
            folder_id: folderId,
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent style={{ maxWidth: 585 }}>
        <AlertDialogHeader>
          <AlertDialogTitle>Upload Video Overlays</AlertDialogTitle>
          <AlertDialogDescription>
            Upload up to {MAX_FILES} video overlay files. Files will be processed in batches of {CONCURRENT_UPLOADS}.
          </AlertDialogDescription>
        </AlertDialogHeader>
