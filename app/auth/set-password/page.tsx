...
import { LockKeyIcon, EyeIcon, CheckmarkCircle01Icon } from "@hugeicons/core-free-icons"
...
function SetPasswordPageContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      setError("Setup token is missing. Please use the link from your email.")
      return
    }
    ...
    try {
      // Use the same reset password endpoint - it works for initial setup too
      ...
      if (response.success) {
        ...
        setTimeout(() => {
          router.push("/auth/login?password_set=true")
        }, 3000)
      }
    } catch (err: any) {
      ...
      if (err.errors) {
        ...
        setError(firstError || err.message || "Failed to set password. Please check your input.")
      } else {
        setError(err.message || "Failed to set password. The link may have expired. Please contact support.")
      }
    } finally {
      setIsLoading(false)
    }
