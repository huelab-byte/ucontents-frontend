...
import { bgmLibraryService, toast, type BgmStats } from "@/lib/api"
...
export default function BgmLibraryPage() {
  ...
  const [stats, setStats] = React.useState<BgmStats | null>(null)
  ...
  const loadStats = React.useCallback(async () => {
    try {
      const response = await bgmLibraryService.getAdminStats()
      if (response.success && response.data) {
  ...
  const handleCreateLibrary = async (library: Pick<Library, "name" | "parent_id">) => {
    try {
      const response = await bgmLibraryService.createFolder({
        name: library.name,
  ...
  const handleUpdateLibrary = async (id: number, updates: Pick<Library, "name" | "parent_id">) => {
    try {
      const response = await bgmLibraryService.updateFolder(id, {
        name: updates.name,
  ...
  const handleView = (id: number) => {
    router.push(`/admin/bgm-library/${id}`)
  }
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Organize and manage your background music assets - categorize and store music files for use in video creation workflows
          </p>
        </div>
          ...
          <Card className="p-12 text-center mr-0 sm:mr-[26px]">
            <CardContent className="pt-6 flex flex-col items-center">
              ...
              <p className="text-muted-foreground mb-4">
                Create your first folder to organize your BGM assets.
              </p>
