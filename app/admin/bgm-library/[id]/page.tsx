...
import { bgmLibraryService, type Bgm, type BgmFolder, type BgmUserWithUploadCount, toast } from "@/lib/api"
...
export default function LibraryViewPage() {
  ...
  const [usersWithUploads, setUsersWithUploads] = React.useState<BgmUserWithUploadCount[]>([])
  ...
  const hasAccess =
    hasPermission("view_bgm") || hasPermission("manage_bgm")
  ...
  const mapBgmToClip = React.useCallback((bgm: Bgm): AudioClip => {
    const filename =
      bgm.title?.trim() ||
      bgm.storage_file?.path?.split("/").pop() ||
      `BGM ${bgm.id}`
    return {
      id: String(bgm.id),
      filename,
      status: bgm.status,
      audioUrl: bgm.storage_file?.url ?? null,
    }
  }, [])

  const findFolderById = React.useCallback((folders: BgmFolder[], id: number): BgmFolder | null => {
  ...
  const loadLibrary = React.useCallback(async () => {
    ...
    try {
      const response = await bgmLibraryService.listFolders()
  ...
  const loadClips = React.useCallback(async () => {
    ...
    try {
      ...
      const response = selectedUserId
        ? await bgmLibraryService.listBgmAdmin(params)
        : await bgmLibraryService.listBgm(params)
      
      if (response.success && response.data) {
        setClips(response.data.map(mapBgmToClip))
  ...
  }, [currentPage, itemsPerPage, libraryId, mapBgmToClip, selectedUserId])
  ...
  const loadUsers = React.useCallback(async () => {
    try {
      const response = await bgmLibraryService.getUsersWithUploads()
      if (response.success && response.data) {
  ...
  const confirmDeleteSelected = async () => {
    ...
    try {
      await Promise.all(
        numericIds.map((id) => bgmLibraryService.deleteBgm(id, true))
      )
      setClips((prev) => prev.filter((clip) => !selectedClips.has(clip.id)))
      setSelectedClips(new Set())
      toast.success("Selected BGM files deleted")
      loadClips()
    } catch (error) {
      console.error("Failed to delete selected BGM:", error)
      toast.error("Failed to delete selected BGM files")
    }
  }
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button
              variant="ghost"
              size="icon"
              onClick={() => router.push("/admin/bgm-library")}
              className="h-8 w-8"
            >
            ...
            <div>
              <h1 className="text-3xl font-bold">{library?.name || "BGM Library"}</h1>
              <p className="text-muted-foreground mt-2">
                Manage your background music files and organize them into folders.
              </p>
            </div>
          </div>
