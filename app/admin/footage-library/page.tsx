...
import { footageLibraryService, toast, type FootageStats } from "@/lib/api"
...
export default function FootageLibraryPage() {
  ...
  const [stats, setStats] = React.useState<FootageStats | null>(null)
  ...
  const loadStats = React.useCallback(async () => {
    try {
      const response = await footageLibraryService.getAdminStats()
      if (response.success && response.data) {
  ...
  const handleCreateLibrary = async (library: Pick<Library, "name" | "parent_id">) => {
    try {
      const response = await footageLibraryService.createFolder({
        name: library.name,
  ...
  const handleUpdateLibrary = async (id: number, updates: Pick<Library, "name" | "parent_id">) => {
    try {
      const response = await footageLibraryService.updateFolder(id, {
        name: updates.name,
  ...
  const confirmDeleteLibrary = async () => {
    ...
    try {
      const response = await footageLibraryService.deleteFolder(deletingLibrary.id)
      if (response.success) {
  ...
  const handleView = (id: number) => {
    router.push(`/admin/footage-library/${id}`)
  }
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Organize and manage your video footage libraries by type - categorize and store landscape (9:16) and vertical (9:16) format assets for efficient content creation workflows
          </p>
        </div>
          ...
          <Card className="p-12 text-center mr-0 sm:mr-[26px]">
            <CardContent className="pt-6 flex flex-col items-center">
              ...
              <p className="text-muted-foreground mb-4">
                Create your first folder to organize your footage assets.
              </p>
