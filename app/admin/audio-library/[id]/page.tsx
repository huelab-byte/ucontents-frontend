...
export default function LibraryViewPage() {
  ...
  const hasAccess =
    hasPermission("view_audio_library") || hasPermission("manage_audio_library")
  ...
  const mapAudioToClip = React.useCallback((audio: Audio): AudioClip => {
    const filename =
      audio.title?.trim() ||
      audio.storage_file?.path?.split("/").pop() ||
      `Audio ${audio.id}`
    return {
      id: String(audio.id),
      filename,
      status: audio.status,
      audioUrl: audio.storage_file?.url ?? null,
    }
  }, [])

  const findFolderById = React.useCallback((folders: AudioFolder[], id: number): AudioFolder | null => {
  ...
  const loadLibrary = React.useCallback(async () => {
    ...
    try {
      const response = await audioLibraryService.listFolders()
  ...
  const loadClips = React.useCallback(async () => {
    ...
    try {
      ...
      const response = selectedUserId
        ? await audioLibraryService.listAudioAdmin(params)
        : await audioLibraryService.listAudio(params)
      
      if (response.success && response.data) {
        setClips(response.data.map(mapAudioToClip))
  ...
  }, [currentPage, itemsPerPage, libraryId, mapAudioToClip, selectedUserId])
  ...
  const loadUsers = React.useCallback(async () => {
    try {
      const response = await audioLibraryService.getUsersWithUploads()
      if (response.success && response.data) {
  ...
  const confirmDeleteSelected = async () => {
    ...
    try {
      await Promise.all(
        numericIds.map((id) => audioLibraryService.deleteAudio(id, true))
      )
      setClips((prev) => prev.filter((clip) => !selectedClips.has(clip.id)))
      setSelectedClips(new Set())
      toast.success("Selected audio files deleted")
    } catch (error) {
      console.error("Failed to delete selected audio:", error)
      toast.error("Failed to delete selected audio files")
    }
  }
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold">{library?.name || "Audio Library"}</h1>
              <p className="text-muted-foreground mt-2">
                Manage your audio files and organize them into folders.
              </p>
            </div>
          </div>
