...
export default function AudioLibraryPage() {
  ...
  const loadStats = React.useCallback(async () => {
    try {
      const response = await audioLibraryService.getAdminStats()
      if (response.success && response.data) {
  ...
  const handleCreateLibrary = async (library: Pick<Library, "name" | "parent_id">) => {
    try {
      const response = await audioLibraryService.createFolder({
        name: library.name,
  ...
  const handleUpdateLibrary = async (id: number | string, updates: Pick<Library, "name" | "parent_id">) => {
    try {
      const response = await audioLibraryService.updateFolder(Number(id), {
        name: updates.name,
  ...
  const confirmDeleteLibrary = async () => {
    ...
    try {
      const response = await audioLibraryService.deleteFolder(Number(deletingLibrary.id))
      if (response.success) {
  ...
  const handleView = (id: number | string) => {
    router.push(`/admin/audio-library/${id}`)
  }
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Organize and manage your audio assets - categorize and store music, sound effects, and voice-overs for efficient content creation workflows
          </p>
        </div>
          ...
          <Card className="p-12 text-center mr-0 sm:mr-[26px]">
            <CardContent className="pt-6 flex flex-col items-center">
              ...
              <p className="text-muted-foreground mb-4">
                Create your first folder to organize your audio assets.
              </p>
