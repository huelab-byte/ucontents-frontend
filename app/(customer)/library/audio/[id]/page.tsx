...
import type { Audio } from "@/lib/api/services/audio-library.service"
import { toast } from "@/lib/api"
import { DeleteConfirmDialog } from "@/components/ui/delete-confirm-dialog"

export default function LibraryAudioFolderPage() {
  ...
  const canUpload = hasPermission("upload_audio")
  const canManage = hasPermission("manage_audio")
  ...
  const mapAudioToClip = React.useCallback((audio: Audio): AudioClip => {
    const filename =
      audio.title?.trim() ||
      (audio.storage_file as { path?: string })?.path?.split("/").pop() ||
      `Audio ${audio.id}`
    return {
      id: String(audio.id),
      filename,
      status: audio.status,
      audioUrl: (audio.storage_file as { url?: string | null })?.url ?? null,
    }
  }, [])
  ...
  const loadClips = React.useCallback(async () => {
    ...
    try {
      setIsLoading(true)
      const response = await audioLibraryService.browseAudio({
        folder_id: libraryId,
        per_page: itemsPerPage,
        page: currentPage,
        status: "ready",
      ...
      if (response.success && response.data) {
        setClips((response.data as Audio[]).map(mapAudioToClip))
  ...
  }, [libraryId, currentPage, mapAudioToClip])
  ...
  const confirmDeleteSelected = async () => {
    ...
    try {
      await Promise.all(ids.map((id) => audioLibraryService.deleteAudio(id, true)))
      setClips((prev) => prev.filter((c) => !selectedClips.has(c.id)))
      setSelectedClips(new Set())
      toast.success("Selected audio deleted")
    } catch (error) {
      console.error("Failed to delete audio:", error)
      toast.error("Failed to delete selected audio")
    }
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            size="icon"
            onClick={() => router.push("/library/audio")}
            className="h-8 w-8"
          >
            <HugeiconsIcon icon={ArrowLeft02Icon} className="size-4" />
            <span className="sr-only">Back to audio library</span>
          </Button>
          <div>
            <h1 className="text-3xl font-bold">{folderName ?? "Audio Library"}</h1>
            <p className="text-muted-foreground mt-2">Browse and use shared audio in this folder.</p>
          </div>
        </div>
        {canUpload && (
